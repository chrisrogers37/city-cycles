...
- **Rate Limiting** - Added slowapi rate limiting middleware to all API endpoints. Default: 60 requests/minute per IP, configurable via `RATE_LIMIT` env var. Returns HTTP 429 with `Retry-After` and `X-RateLimit-*` headers when exceeded; `/health` (the Railway healthcheck path) is exempt. Per-IP keying works behind Railway's edge: the API container runs uvicorn with `--proxy-headers --forwarded-allow-ips` trusting only Railway's internal peer block (`FORWARDED_ALLOW_IPS`, default `100.64.0.0/10`), so visitors get separate buckets and client-spoofed `X-Forwarded-For` entries are ignored.
