...
class DuckDBOperations:
        ...
        try:
            ...
            for table_name in expected_tables:
                try:
                    ...
                    logger.info(
                        f"✓ Table {table_name}: {table_info['row_count']} rows, {table_info['size_mb']} MB"
                    )
                except Exception as e:
    ...
    def load_data(
        ...
        try:
            with DuckDBManager(db_path=self.db_path) as db:
                ...
                tables_to_load = (
                    {table_name: S3_URIS[table_name]} if table_name else S3_URIS
                )
                ...
                for table_name, s3_uri in tables_to_load.items():
                    ...
                    try:
                        if dry_run:
                            logger.info(
                                f"[DRY RUN] Would load {table_name} from {s3_uri}"
                            )
    ...
    def verify_data(
        self, table_name: Optional[str] = None, detailed: bool = False
    ) -> Dict[str, Dict]:
        """
    ...
    def _verify_table_data(
        self, table_name: str, db: DuckDBManager, detailed: bool = False
    ) -> Dict:
        ...
        try:
            ...
            logger.error(f"Failed to verify table {table_name}: {e}")
            return {"table_name": table_name, "status": "FAIL", "error": str(e)}
    ...
    def _run_data_quality_checks(self, table_name: str, db: DuckDBManager) -> Dict:
        ...
        try:
            # 1. Null checks
            null_cases = ", ".join(
                f"SUM(CASE WHEN {self._validate_identifier(col)} IS NULL THEN 1 ELSE 0 END) as null_{self._validate_identifier(col)}"
                for col in config["null_check_columns"]
            )
            null_query = (
                f"SELECT COUNT(*) as total_rows, {null_cases} FROM {safe_table}"
            )
            ...
            if config["duplicate_key"]:
                key_col = self._validate_identifier(config["duplicate_key"])
                dup_query = f"""
            ...
            # 3. Date range checks
            start_col = self._validate_identifier(config["date_columns"][0])
            end_col = self._validate_identifier(config["date_columns"][1])
            date_query = f"""
                SELECT
    ...
    def _generate_summary_report(self, verification_results: List[Dict]) -> str:
        ...
        report.append(
            f"Tables passed: {len(verification_results) - len(failed_tables)}"
        )
    ...
    def export_marts(
        ...
        INTERMEDIATE_TABLES = ["int_london_rides", "int_nyc_rides", "unified_rides"]
        ...
        try:
            with DuckDBManager(db_path=self.db_path) as db:
                ...
                if table_name:
                    ...
                    tables_to_export = MART_TABLES + (
                        INTERMEDIATE_TABLES if include_intermediate else []
                    )
                ...
                for table_name in tables_to_export:
                    ...
                    success = self._export_table_to_s3(
                        db, full_table_name, s3_key, dry_run
                    )
                    results[table_name] = success

                    if not success and not dry_run:
                        logger.error(
                            f"Failed to export {table_name}, stopping export process"
                        )
                        break
                ...
                if not dry_run:
                    ...
                    logger.info(f"\n{'=' * 50}")
                    logger.info(
                        f"✓ EXPORT COMPLETED: {successful_exports}/{total_exports} tables exported"
                    )
                    logger.info(f"{'=' * 50}")
                else:
                    logger.info(f"\n{'=' * 50}")
                    logger.info(
                        f"✓ DRY RUN COMPLETED: {len(tables_to_export)} tables would be exported"
                    )
                    logger.info(f"{'=' * 50}")
    ...
    def _export_table_to_s3(
        self, db: DuckDBManager, table_name: str, s3_key: str, dry_run: bool = False
    ) -> bool:
        ...
        try:
            ...
            if dry_run:
                logger.info(
                    f"[DRY RUN] Would export {table_name} ({row_count:,} rows) to s3://{S3_BUCKET}/{s3_key}"
                )
                return True
            ...
            logger.info(
                f"Exporting {table_name} ({row_count:,} rows) to s3://{S3_BUCKET}/{s3_key}"
            )
            db.con.execute(export_sql)

            logger.info(
                f"✓ Successfully exported {table_name} to s3://{S3_BUCKET}/{s3_key}"
            )
            return True
